...
from engine.extre_module.custom_nn.module.LSCA import LSCA
from engine.extre_module.custom_nn.featurefusion.cgfm import ContextGuideFusionModule
from engine.extre_module.custom_nn.downsample.awds import AWDS
...
def parse_module(d, i, f, m, args, ch, nc=None, eval_spatial_size=None):
    ...
    elif m in {InceptionDWBlock, GlobalBlock, LocalBlock, OPE, LEGBlock, MSCB, ARF, LEGM, CAMixer, Heat2D, UniRepLKNetBlock, Star_Block, RepViTBlock, RepConvBlock, PKIBlock, MSBlock, MambaOut,MambaOut_DilatedReparamConv,MambaOut_UniRepLKBlock,
        LFE, JDPM, iRMB, FMB, FCM, FAT_Block, Faster_Block,Faster_Block_CGLU, ESCBlock, ELGCA_EncoderBlock, EfficientViMBlock,EfficientViMBlock_CGLU, EBlock, DynamicFilter, DWR, DBlock, APBottleneck, DPLKA, LSCA,MSDAM,CFBlock}: # module
        c1, c2 = ch[f], args[0]     
        args = [c1, c2, *args[1:]]    
    elif m in {MANet}: # Block   
        c1, c2 = ch[f], args[0]
        if selfatt is not None:
            args = ([c1, c2, *args[1:]], {'selfatt':selfatt})    
        else:
            args = [c1, c2, *args[1:]]      
    elif m in {ContextGuideFusionModule}: # featurefusion    
        c1 = [ch[i] for i in f]
        c2 = args[0]     
        args = [c1, c2, *args[1:]]
    elif m in {AWDS}: # DownSample
        c1, c2 = ch[f], args[0]  
        args = [c1, c2, *args[1:]]  
